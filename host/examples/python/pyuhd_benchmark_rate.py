...
def pp_now():
    ...
    return "{:%H:%M:%S.%f}".format(datetime.now())
...
def setup_ref(usrp, ref, num_mboards):
    ...
    if ref == "mimo":
        if num_mboards != 2:
            logging.error("ref = \"mimo\" implies 2 motherboards; "
    ...
    if ref != "internal":
        logging.debug("Now confirming lock on clock signals...")
        ...
        for i in range(num_mboards):
            ...
            while (not is_locked) and (datetime.now() < end_time):
                ...
                is_locked = usrp.get_mboard_sensor("ref_locked", i)
            if not is_locked:
                logging.error("Unable to confirm clock signal locked on board %d", i)
                return False
    return True
...
def check_channels(usrp, args):
    ...
    if args.rx_rate:
        ...
        if not all(map((lambda chan: chan < dev_rx_channels), rx_channels)):
            logging.error("Invalid RX channel(s) specified.")
            return [], []
    else:
        rx_channels = []
    # Check TX channels
    if args.tx_rate:
        ...
        if not all(map((lambda chan: chan < dev_tx_channels), tx_channels)):
            logging.error("Invalid TX channel(s) specified.")
            return [], []
    else:
        tx_channels = []
...
def benchmark_rx_rate(usrp, rx_streamer, random, timer_elapsed_event, rx_statistics):
    ...
    logging.info("Testing receive rate {:.3f} Msps on {:d} channels".format(
    ...
    while not timer_elapsed_event.is_set():
        ...
        try:
            ...
            logging.error("Runtime error in receive: %s", ex)
            return
        ...
        if metadata.error_code == uhd.types.RXMetadataErrorCode.none:
            ...
            if metadata.out_of_sequence:
                num_rx_seqerr += 1
                logging.warning("Detected RX sequence error.")
            # Otherwise just count the overrun
            else:
                num_rx_overruns += 1
        elif metadata.error_code == uhd.types.RXMetadataErrorCode.late:
            logging.warning("Receiver error: %s, restarting streaming...", metadata.strerror())
            num_rx_late += 1
            ...
            logging.warning("Receiver error: %s, continuing...", metadata.strerror())
            num_rx_timeouts += 1
        else:
            logging.error("Receiver error: %s", metadata.strerror())
            logging.error("Unexpected error on receive, continuing...")
...
def benchmark_tx_rate(usrp, tx_streamer, random, timer_elapsed_event, tx_statistics):
    ...
    logging.info("Testing transmit rate %.3f Msps on %d channels",
        ...
        if random:
            ...
            while num_acc_samps < total_num_samps:
                num_tx_samps += tx_streamer.send(
                    transmit_buffer, max_samps_per_packet, metadata, timeout) * num_channels
            ...
            try:
                ...
                if num_tx_samps_now == 0:
                    ...
                    if (num_timeouts_tx % 10000) == 1:
                        logging.warning("Tx timeouts: %d", num_timeouts_tx)
                metadata.has_time_spec = False

            except RuntimeError as ex:
                logging.error("Runtime error in transmit: %s", ex)
                return
    ...
    try:
        while not timer_elapsed_event.is_set():
            ...
            if async_metadata.event_code == uhd.types.TXMetadataEventCode.burst_ack:
                ...
                logging.warning("Unexpected event on async recv (%s), continuing.",
...
def print_statistics(rx_statistics, tx_statistics, tx_async_statistics):
    ...
    logging.debug("RX Statistics Dictionary: %s", rx_statistics)
    logging.debug("TX Statistics Dictionary: %s", tx_statistics)
    logging.debug("TX Async Statistics Dictionary: %s", tx_async_statistics)
...
def main():
    ...
    if not (args.rx_rate or args.tx_rate):
        logging.error("Please specify --rx_rate and/or --tx_rate")
        return False
    ...
    if args.rx_subdev:
        usrp.set_rx_subdev_spec(args.rx_subdev)
    if args.tx_subdev:
        usrp.set_tx_subdev_spec(args.tx_subdev)

    logging.info("Using Device: %s", usrp.get_pp_string())
    ...
    if not rx_channels and not tx_channels:
        ...
        return False
    logging.info("Selected %s RX channels and %s TX channels",
    ...
    logging.debug("Sending signal to stop!")
    quit_event.set()
    for thr in threads:
        thr.join()
...
if __name__ == "__main__":
    logging.basicConfig(level=logging.DEBUG,
                        format='[{}] [%(levelname)s] (%(threadName)-10s) %(message)s'.format(
                            pp_now()),)
    sys.exit(not main())
